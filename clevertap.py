...
def _convert_to_csv(result, pathcsv,batch_count):
    result_list = []
    main_list_of_fields = []
    ...
    print("Converted And stored Batch "+batch_count+" in CSV File")
    ...
    def _fetch_records(self, type, query, batch_size=10):
        ...
        if self.cursor:
            ...
            def call_records():
                ...
                batch_count +1
                _convert_to_csv(new_records, "March_P_AXMB.csv",batch_count)

                # add the new records counts to our records count
                self.records += len(new_records)
